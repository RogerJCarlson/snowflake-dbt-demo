...
--    ,VISIT_DETAIL.VISIT_DETAIL_ID AS VISIT_DETAIL_ID
	...
	,LEFT(CAST( DEVICE_EXPOSURE_CLARITYHOSP_LDA.FLO_MEAS_ID AS VARCHAR) 
			|| ': ' || DEVICE_EXPOSURE_CLARITYHOSP_LDA.FLO_MEAS_NAME,50) 
		AS DEVICE_SOURCE_VALUE
	,0 AS DEVICE_SOURCE_CONCEPT_ID
	,'DEVICE_EXPOSURE--CLARITYHOSP--LDA' AS ETL_MODULE
	,VISIT_SOURCE_VALUE

FROM {{ref('DEVICE_EXPOSURE_CLARITYHOSP_LDA')}} AS DEVICE_EXPOSURE_CLARITYHOSP_LDA
	
    INNER JOIN {{ source('CDM','SOURCE_TO_CONCEPT_MAP')}} AS T_SOURCE_TO_CONCEPT_MAP_DEVICE_LDA
          ON DEVICE_EXPOSURE_CLARITYHOSP_LDA.FLO_MEAS_ID::VARCHAR = T_SOURCE_TO_CONCEPT_MAP_DEVICE_LDA.SOURCE_CODE
