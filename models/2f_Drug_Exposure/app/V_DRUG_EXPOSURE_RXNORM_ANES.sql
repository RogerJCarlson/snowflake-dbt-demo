...
FROM {{ref('DRUG_EXPOSURE_CLARITYANES_RXNORM')}} AS DRUG_EXPOSURE_CLARITYANES_RXNORM
...
LEFT JOIN {{ source('CDM','SOURCE_TO_CONCEPT_MAP')}} AS  SOURCE_TO_CONCEPT_MAP_ROUTE
	ON (SOURCE_TO_CONCEPT_MAP_ROUTE.SOURCE_VOCABULARY_ID) = 'SH_route'
		AND SOURCE_TO_CONCEPT_MAP_ROUTE.SOURCE_CODE::VARCHAR = DRUG_EXPOSURE_CLARITYANES_RXNORM.ROUTE_C::VARCHAR
...
INNER JOIN -- THIS REMOVES THE DUPLICATES 
	...
	FROM {{ref('TOP_RXNORM_ANES')}} AS TOP_RXNORM
