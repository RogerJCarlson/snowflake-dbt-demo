--V_DRUG_EXPOSURE_RXNORM_AMB

{{ config(materialized = 'view') }}

SELECT DISTINCT 
--	 CDM.SEQ_DRUG_EXPOSURE.NEXTVAL AS DRUG_EXPOSURE_ID
	 DRUG_EXPOSURE_CLARITYAMB_RXNORM.PERSON_ID
	,T_DRUG_CONCEPT.DRUG_CONCEPT_ID
	,CAST( ORDER_START_TIME AS DATE) AS DRUG_EXPOSURE_START_DATE
	,ORDER_START_TIME AS DRUG_EXPOSURE_START_DATETIME
	,CAST( COALESCE(ORDER_END_TIME, ORDER_START_TIME)AS DATE) AS DRUG_EXPOSURE_END_DATE
	,CASE 
		WHEN (CAST( COALESCE(ORDER_END_TIME, ORDER_START_TIME)AS DATE) = CAST( ORDER_START_TIME AS DATE))
...
--    ,VISIT_DETAIL.VISIT_DETAIL_ID AS VISIT_DETAIL_ID
	...
	,CAST( DRUG_EXPOSURE_CLARITYAMB_RXNORM.MEDICATION_ID AS VARCHAR) || ':' || LEFT(CLARITY_MEDICATION_NAME, 49 - LEN(DRUG_EXPOSURE_CLARITYAMB_RXNORM.MEDICATION_ID)) AS DRUG_SOURCE_VALUE
	,T_DRUG_SOURCE.CONCEPT_ID AS DRUG_SOURCE_CONCEPT_ID
	,CAST( DRUG_EXPOSURE_CLARITYAMB_RXNORM.MED_ROUTE_C AS VARCHAR) || ':' || LEFT(ZC_ADMIN_ROUTE_NAME, 49 - LEN(DRUG_EXPOSURE_CLARITYAMB_RXNORM.MED_ROUTE_C)) AS ROUTE_SOURCE_VALUE
	,ZC_MED_UNIT_NAME AS DOSE_UNIT_SOURCE_VALUE
	,'DRUG_EXPOSURE--CLARITYAMB--RXNORM' AS ETL_MODULE
	,VISIT_SOURCE_VALUE

FROM {{ref('DRUG_EXPOSURE_CLARITYAMB_RXNORM')}} AS DRUG_EXPOSURE_CLARITYAMB_RXNORM

    INNER JOIN {{ref('VISIT_OCCURRENCE')}} AS VISIT_OCCURRENCE
    	ON DRUG_EXPOSURE_CLARITYAMB_RXNORM.PAT_ENC_CSN_ID = VISIT_OCCURRENCE.VISIT_SOURCE_VALUE
    
    LEFT JOIN {{ref('PROVIDER')}} AS PROVIDER
    	ON DRUG_EXPOSURE_CLARITYAMB_RXNORM.AUTHRZING_PROV_ID = PROVIDER.PROVIDER_SOURCE_VALUE

    INNER JOIN {{ref('TOP_RXNORM_AMB')}} AS TOP_RXNORM
    	ON DRUG_EXPOSURE_CLARITYAMB_RXNORM.MEDICATION_ID = TOP_RXNORM.MEDICATION_ID
    ...
    LEFT JOIN {{ source('CDM','SOURCE_TO_CONCEPT_MAP')}} AS  SOURCE_TO_CONCEPT_MAP_ROUTE
    	ON SOURCE_TO_CONCEPT_MAP_ROUTE.SOURCE_CODE = DRUG_EXPOSURE_CLARITYAMB_RXNORM.MED_ROUTE_C
    		AND (SOURCE_TO_CONCEPT_MAP_ROUTE.SOURCE_VOCABULARY_ID) = 'SH_route'

    INNER JOIN
		{{ source('CDM','SOURCE_TO_CONCEPT_MAP')}} AS  SOURCE_TO_CONCEPT_MAP_AMB_VISIT
		ON	SOURCE_TO_CONCEPT_MAP_AMB_VISIT.SOURCE_CODE   = DRUG_EXPOSURE_CLARITYAMB_RXNORM.ENC_TYPE_C
    ...
    INNER JOIN -- THIS REMOVES THE DUPLICATES 
    	...
    	FROM {{ref('TOP_RXNORM_AMB')}} AS TOP_RXNORM
...
WHERE DRUG_EXPOSURE_CLARITYAMB_RXNORM.ORDER_START_TIME IS NOT NULL
