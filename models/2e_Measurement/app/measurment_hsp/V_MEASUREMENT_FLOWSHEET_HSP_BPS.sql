--V_MEASUREMENT_FLOWSHEET_HSP_BPS
...
SELECT DISTINCT --CDM.SEQ_MEASUREMENT.NEXTVAL AS MEASUREMENT_ID
	...
	, CAST( LEFT(MEAS_VALUE, CHARINDEX('/', MEAS_VALUE) - 1) AS FLOAT) AS VALUE_AS_NUMBER --SYSTOLIC
...
--    ,VISIT_DETAIL.VISIT_DETAIL_ID AS VISIT_DETAIL_ID
	...
	,'FLWSHT: ' || MEASUREMENT_CLARITYHOSP_FLOWSHEET.FLO_MEAS_NAME AS MEASUREMENT_SOURCE_VALUE
	,0 AS MEASUREMENT_SOURCE_CONCEPT_ID
	,SOURCE_TO_CONCEPT_MAP_FLOWSHEET_MEAS_UNITS.SOURCE_CODE_DESCRIPTION AS UNIT_SOURCE_VALUE
	,MEASUREMENT_CLARITYHOSP_FLOWSHEET.MEAS_VALUE AS VALUE_SOURCE_VALUE
	,'MEASUREMENT--CLARITYHOSP--FLOWSHEET_BPS' AS ETL_MODULE
	,VISIT_SOURCE_VALUE
...
FROM {{ref('MEASUREMENT_CLARITYHOSP_FLOWSHEET')}} AS MEASUREMENT_CLARITYHOSP_FLOWSHEET
	...
	INNER JOIN {{ source('CDM','SOURCE_TO_CONCEPT_MAP')}} AS   SOURCE_TO_CONCEPT_MAP_FLOWSHEET_MEAS
		ON MEASUREMENT_CLARITYHOSP_FLOWSHEET.FLO_MEAS_ID = SOURCE_TO_CONCEPT_MAP_FLOWSHEET_MEAS.SOURCE_CODE
			AND UPPER(SOURCE_TO_CONCEPT_MAP_FLOWSHEET_MEAS.SOURCE_VOCABULARY_ID) = 'SH_FLWSHT_MEAS_BPS'

	LEFT JOIN {{ source('CDM','SOURCE_TO_CONCEPT_MAP')}} AS   SOURCE_TO_CONCEPT_MAP_FLOWSHEET_MEAS_UNITS
		ON 
			MEASUREMENT_CLARITYHOSP_FLOWSHEET.FLO_MEAS_ID  = SOURCE_TO_CONCEPT_MAP_FLOWSHEET_MEAS_UNITS.SOURCE_CODE 
